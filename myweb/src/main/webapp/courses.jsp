...
<table border=1>
    ...
    try (Connection conn=DriverManager.getConnection(CONNECTION_STRING)){ //Открываем соединение с БД
        ...
        //String sql="select title,length,id,description from courses where title like ? or description like ? order by title ";            
        //PreparedStatement cmd=conn.prepareStatement(sql);//создаем коменду для выполнения селекта
        ...
        String sql="select title,length,id,description from courses where INSTR(title,?)>0 or INSTR(description,?) order by title";            
        //title=Основы java
        //INSTR(title,"java")=8-важно понять что строка есть а не количество вхождений поэтому >0		
        PreparedStatement cmd=conn.prepareStatement(sql);
        cmd.setString(1,search);
        try(ResultSet result=cmd.executeQuery()){ //result это курсор кот позволяет последовательно перебрать все строки результата
            while(result.next()){
